...
class Recipe extends BaseController
{
    ...
    public function edit($id_recipe) {
        ...
        //Récupérer les informations de la recette (mêmes désactivée)
    ...
    public function update() {
        ...
        if($rm->update($id_recipe,$data)){
            ...
            $qm = Model('QuantityModel');
            //Suppression des ingrédients de la table quantity pour la recette s'il y en a
            $qm->where('id_recipe',$id_recipe)->delete();
            //insertion des ingrédients pour la recette
            if(isset($data['ingredients'])) {
                //Ajout des ingrédients
                foreach($data['ingredients'] as $ingredient) {
                    $ingredient['id_recipe']=$id_recipe;
                    if ($qm->insert($ingredient)) {
                        $this->success('Ingrédient modifié avec succès !');
                    } else {
                        foreach($qm->errors() as $error){
                            $this->error($error);
                        }
    ...
    public function switchActive() {
        ...
        // Récupérer la recette (même si elle est soft deleted)
        $recipe = $rm->withDeleted()->find($id);
